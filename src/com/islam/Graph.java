...
public class Graph {
    ...
    public void traverseDepthFirstRecursive(Node root, Set<Node> visited) {
        System.out.println(root);
        visited.add(root);

        for (var node : adjacencyList.get(root)) {
            if (!visited.contains(node)) traverseDepthFirstRecursive(node, visited);
        }
    }
    ...
    public void topologicalSort(Node node, Set<Node> visited, Stack<Node> stack) {
        ...
        for (var neighbor : adjacencyList.get(node)) topologicalSort(neighbor, visited, stack);
